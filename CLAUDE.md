...
### WhatsApp automation was disabled — quote delivery is PDF-download-and-send-manually only

`backend/src/modules/whatsapp/` (client/service) and the `orcamentos.service.ts` send path (`enviarOrcamentoPorWhatsApp`) still exist in the codebase but are **dormant and unreachable from the UI**. `backend/src/server.ts` no longer calls `initWhatsApp()` at boot, so `whatsappClient` never calls `.initialize()` and never launches its own Chromium — the "Enviar orçamento por WhatsApp" button, the `/configuracoes/whatsapp` QR-connect page, and the `/api/whatsapp/*` routes were all removed from the frontend/`app.ts`. This was a deliberate rollback, not a bug: the automation (whatsapp-web.js, unofficial — not the Meta Business API) repeatedly failed on Render's free tier in production — first with pairing failures ("não foi possível conectar o dispositivo", usually a stale bundled WhatsApp Web protocol version), then with WhatsApp's own anti-abuse system actively logging out the freshly-linked session (`disconnected` reason `LOGOUT`) after a few repeated pairing attempts from the datacenter IP, on top of its separate Chromium instance visibly starving the PDF-generation Chromium for memory on the 512MB free-tier container. If WhatsApp automation is ever revived, budget for the official Meta Cloud API (or a BSP like Twilio/Zenvia/360dialog) instead of re-enabling whatsapp-web.js — see the exploration in chat history for the trade-offs (business verification, template approval, per-conversation billing).

Because of this, `orcamentos.service.ts`'s `gerarPdfOrcamento()` (used by the "Baixar PDF do orçamento" button — the only delivery path left) now generates/renews the client-authorization token itself and always embeds `linkAutorizacao` in the rendered PDF; before the rollback only the WhatsApp-send path did this, which meant a downloaded-and-manually-sent PDF had no way for the client to approve/reject. If you ever touch `gerarPdfOrcamento`, don't drop that token generation — it's not optional anymore now that it's the sole delivery mechanism.

If you re-enable `initWhatsApp()` in `server.ts` in the future: root `package.json` still has `"overrides": { "puppeteer": "^22.15.0" }` so whatsapp-web.js's own nested `puppeteer` dependency resolves to the same version as the top-level one instead of installing/downloading a second Chromium, and `whatsapp.client.ts` still points its `puppeteer.executablePath` at `chromiumExecutablePath()` from `pdf.engine.ts` — keep both in sync if you bump the `puppeteer` version. `pdf.engine.ts`'s `getBrowser()` now self-heals (clears its cached launch promise on failure/disconnect and relaunches on the next call) rather than staying permanently broken after one crash — this was a real bug found when PDF generation kept failing with a bare 500 after the WhatsApp Chromium instance had exhausted the container's memory; keep this behavior if you touch that function.

**Never hardcode `PUPPETEER_EXECUTABLE_PATH` in the Docker build.** `chromiumExecutablePath()` (still called from `whatsapp.client.ts` at module import time — the `Client` constructor evaluates it as an argument even though `.initialize()` is never invoked — i.e. before `server.ts` ever reaches `app.listen()`) calls `puppeteer.executablePath()`, which throws synchronously if the configured path doesn't actually exist on disk — crashing the whole process with no HTTP server ever bound and no log line, which on Render just looks like "no open ports detected" until the deploy times out. `backend/Dockerfile` deliberately does **not** set `PUPPETEER_EXECUTABLE_PATH`/`PUPPETEER_SKIP_CHROMIUM_DOWNLOAD` even though it's built `FROM ghcr.io/puppeteer/puppeteer` — that image manages its own Chrome install path internally (not at the commonly-assumed `/usr/bin/google-chrome-stable`), so `npm ci` is left to download Chrome itself via puppeteer's normal postinstall, guaranteeing `executablePath()` resolves to something that actually exists.
...
time-boxed token row), just with a 30-minute expiry instead of 15 days, and was built to
deliver its link via `whatsapp.service.ts`'s `sendTexto()`. Since WhatsApp automation was
disabled (see above), this send always fails silently — `isWhatsAppConnected()` is now
permanently `false`, so `sendTexto()` always throws `WhatsAppNaoConectadoError`, which
`solicitarRedefinicao()` in `auth.service.ts` swallows and logs server-side by design
(it never reveals to the caller whether the email/phone exists, to prevent account
enumeration — see that function). The frontend flow (`EsqueciSenhaPage.tsx`,
`RedefinirSenhaPage.tsx`) is still wired up and shows the same generic "check your
WhatsApp" message, but the message never arrives. **Until this is replaced with a real
channel (email is the obvious candidate — see the exploration in chat history), the only
way to reset a user's password is an Admin editing them directly in Usuários** (leave
every other field as-is, fill just `senha`, save — `usuarios.service.ts`'s `atualizar()`
already supports this).

### Process-level crash resilience for out-of-band async failures

`backend/src/server.ts` registers `process.on("unhandledRejection", ...)` and `process.on("uncaughtException", ...)` that log and deliberately do **not** exit the process. This was originally added because `whatsapp-web.js` fired puppeteer-internal async operations outside the normal request/response cycle (session keep-alive, auth checks) that could reject without any local `.catch()`; it's kept in place now that WhatsApp automation is disabled because `pdf.engine.ts`'s Puppeteer instance has the same class of risk (page/browser-level async failures outside a request's own promise chain). Node's default behavior is to crash the entire process on an unhandled rejection, which would take the whole REST API down over an isolated Puppeteer hiccup. Route-level errors are unaffected by this — they're already caught by `asyncHandler` and never reach these global handlers; these are purely a safety net for rejections outside the Express lifecycle.

### `getNumberId` note (dormant code, kept for if WhatsApp automation is ever revived)

`backend/src/modules/whatsapp/whatsapp.service.ts` used to build chat ids as `` `${digits}@c.us` `` directly; this broke for real-world sends with `Error: No LID for user` once WhatsApp rolled out its LID (Linked ID) identity system — a phone-number-based JID that hasn't been resolved through the client first isn't a valid `sendMessage` target anymore. `resolveChatId()` calls `whatsappClient.getNumberId(digits)` and throws a `ValidationError` with a clear PT-BR message if it resolves to `null` (number not on WhatsApp) instead of letting the raw library exception surface. This code is currently unreachable (see above) but don't revert to manual `@c.us` string construction if it's ever reconnected.
...
- **`npm run dev:backend` hangs forever with no "Backend rodando..." log line, or crashes immediately with a Puppeteer/Chrome-not-found stack trace**: `backend/src/modules/whatsapp/whatsapp.client.ts` resolves the Chromium executable path inside the top-level `new Client(...)` call, which runs at **module import time** — before `server.ts` even reaches `app.listen()`. This still happens even though WhatsApp automation is disabled (see above) — `initWhatsApp()`/`.initialize()` is never called, but the `Client` is still constructed, and `chromiumExecutablePath()` is evaluated eagerly as one of its constructor arguments. Puppeteer's Chromium download lives per-Windows-user at `~/.cache/puppeteer` (unrelated to the repo's drive letter), so switching Windows profiles (or a fresh machine) means it's missing and the whole server fails to boot on an import-time exception. Fix: `cd backend && npx puppeteer browsers install chrome`.
- **`npm run dev:backend` fails to restart after an edit, with `EADDRINUSE: address already in use :::3333` and/or `Error: The browser is already running for .../whatsapp-session/session`**: this was specific to when `initWhatsApp()` was still called at boot (now disabled — see above), so it should no longer reproduce in normal dev flow; kept here in case that call is ever restored. `tsx watch` restarting the child process on Windows doesn't always cleanly kill the previous child's Puppeteer/Chromium grandchildren, so they (and the port) can survive the restart and collide with the new one. Symptom check: `Get-Process -Name chrome | Where-Object { $_.Path -like "*\.cache\puppeteer\*" }` in PowerShell — if that lists more than a handful, they're orphaned (don't touch other `chrome.exe` processes at that same path filter's absence; those are the developer's real browser, not Puppeteer's — **never run a bare `taskkill /IM chrome.exe` or `Stop-Process -Name chrome`, it kills the real browser too, path filter is mandatory**). Fix: stop those processes, delete the stale `backend/whatsapp-session/session/lockfile`, then start a single fresh `npm run dev:backend`.
...
- **Backend → Render** (`backend/Dockerfile`, driven by `render.yaml` at repo root), on Render's **free tier** — the service sleeps on inactivity (cold start, mitigated by the keep-alive ping below) and has no persistent disk. The lack of persistent disk no longer matters in practice: it originally existed to keep `WHATSAPP_SESSION_PATH` alive between container restarts, but WhatsApp automation is now disabled (see above), so nothing in the app currently depends on disk surviving a restart. `railway.json` still exists at the repo root as a ready-to-go migration path to a host with persistent disk (Railway), reusing the same Dockerfile, in case a future feature needs it again.
...
Copy `.env.example` → `backend/.env` and `frontend/.env`. Key backend vars: `DATABASE_URL` (Neon/Supabase Postgres connection string), `JWT_SECRET`/`JWT_EXPIRES_IN`, `FRONTEND_URL` (used both to build the client-authorization link embedded in every quote PDF, e.g. `${FRONTEND_URL}/autorizacao/:token`, and — when `NODE_ENV=production` — as the CORS allowlist, comma-separated for multiple origins; see Deployment above), `WHATSAPP_SESSION_PATH` (gitignored; still required by `env.ts`'s validation and still read by the now-dormant `whatsapp.client.ts`, but doesn't do anything observable since WhatsApp automation is disabled — see above), `NODE_ENV` (defaults to `development`; only `production` triggers the CORS restriction, doesn't otherwise change backend behavior). `backend/src/config/env.ts` validates all of this at startup with zod and throws immediately on a missing/invalid var rather than failing later at first use.
